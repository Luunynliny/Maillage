#!/bin/bash
#
# Fetches the FluidAudio streaming ASR model this app embeds — Nemotron Speech Streaming
# Multilingual, "latin" ship (shared vocabulary across en/es/fr/it/pt/de, so French sentences with
# English technical terms decode as one language rather than needing per-word language detection),
# 2240ms chunk tier (FluidAudio's documented recommended default: the latency/accuracy balance for
# a live meeting transcript, not a dictation-speed tool). Called by the "Fetch FluidAudio ASR
...
echo "==> Downloading $variant (~600MB, this only happens once, then it's cached)"
